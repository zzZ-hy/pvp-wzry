module.exports = app => {
  ...
  router.get('/news/init', async (req, res) => {
    ...
    const newsTitles = ["《目标》·新英雄澜CG", "12月8日体验服停机更新公告", "12月8日碎片商店异常问题修复公告", "12月8日碎片商店异常问题说明", "新英雄澜登场，冬日聚峡谷对战赢好礼", "《目标》·新英雄澜CG", "【获奖公示】云阁绮霞 -王者荣耀风物志插画漫画大赛", "新皮肤爆料｜守护万千星河，云上仙君新装登场！", "顺风局主题语音票选活动结果公告", "这大唐盛世如你所愿 《乱世王者》喜迎公测三周年庆"]
  ...
  router.get('/heroes/init',async (req,res)=>{
    ...
    for (let value of rawData){
      ...
      value.heroes = value.heroes.map(hero=>{
        hero.categories = [category]
        return hero
      })

      // 录入英雄
      await Hero.insertMany(value.heroes)
    }
  ...
  // 获取全部英雄数据
  router.get('/heroes/all',async (req,res)=>{
    const allHero = await Hero.find().lean()
    res.send({
      name: '全部',
      _id: 'all1hero',
      all_hero: allHero
